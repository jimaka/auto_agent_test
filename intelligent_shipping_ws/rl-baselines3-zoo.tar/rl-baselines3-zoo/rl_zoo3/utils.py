...
try:
    ...
    class ModelName(str):
        """Fallback string-based model identifier."""

        filename: str

        def __new__(cls, algo: str, env_name: EnvironmentName):
            value = f"{algo}-{env_name}"
            obj = str.__new__(cls, value)
            obj.filename = f"{value}.zip"
            return obj

try:
    from sb3_contrib import ARS, QRDQN, TQC, TRPO, CrossQ, RecurrentPPO
except ImportError:  # pragma: no cover - optional dependency
    ARS = QRDQN = TQC = TRPO = CrossQ = RecurrentPPO = None
...
def get_hf_trained_models(organization: str = "sb3", check_filename: bool = False) -> dict[str, tuple[str, str]]:
    ...
    """
    if HfApi is None:
        raise ImportError("huggingface_hub is required for get_hf_trained_models(). Please install huggingface_hub.")
    api = HfApi()
