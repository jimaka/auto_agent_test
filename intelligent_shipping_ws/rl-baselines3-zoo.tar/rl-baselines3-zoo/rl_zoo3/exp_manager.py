...
try:
    ...
    class EnvironmentName(str):
        """Fallback type when huggingface_sb3 is not installed."""

        gym_id: str

        def __new__(cls, env_id: str):
            obj = str.__new__(cls, env_id)
            obj.gym_id = env_id
            return obj
...
try:
    ...
    BasePruner = BaseSampler = object  # type: ignore[assignment]

    class MedianPruner:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError("optuna is required for hyperparameter optimization.")

    class NopPruner:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError("optuna is required for hyperparameter optimization.")

    class SuccessiveHalvingPruner:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError("optuna is required for hyperparameter optimization.")

    class RandomSampler:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError("optuna is required for hyperparameter optimization.")

    class TPESampler:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError("optuna is required for hyperparameter optimization.")

    class MaxTrialsCallback:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError("optuna is required for hyperparameter optimization.")
    ...
    def plot_optimization_history(*args, **kwargs):  # type: ignore[no-redef]
        raise ImportError("optuna is required for hyperparameter optimization.")

    def plot_param_importances(*args, **kwargs):  # type: ignore[no-redef]
        raise ImportError("optuna is required for hyperparameter optimization.")
...
class ExperimentManager:
    ...
    def __init__(
        ...
        if self.optimize_hyperparameters and not OPTUNA_AVAILABLE:
            raise ImportError("optuna is required when --optimize-hyperparameters is enabled.")
    ...
    def learn(self, model: BaseAlgorithm) -> None:
        ...
        if self.algo == "ars" and self.n_envs > 1:
            if AsyncEval is None:
                raise ImportError("ARS async_eval requires sb3_contrib to be installed.")
