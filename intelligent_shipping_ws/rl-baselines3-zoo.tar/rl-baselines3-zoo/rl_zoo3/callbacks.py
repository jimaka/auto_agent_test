...
class TrialEvalCallback(EvalCallback):
    ...
    def __init__(
        ...
        if optuna is None:
            raise ImportError("optuna is required for TrialEvalCallback.")
        super().__init__(
            eval_env=eval_env,
...
class ParallelTrainCallback(BaseCallback):
    ...
    def _init_callback(self) -> None:
        ...
        # make mypy happy
        supported_models = tuple(cls for cls in (SAC, TQC) if cls is not None)
        assert isinstance(self.model, supported_models), f"{self.model} is not supported for parallel training"
    ...
    def _on_rollout_end(self) -> None:
        # Make mypy happy
        supported_models = tuple(cls for cls in (SAC, TQC) if cls is not None)
        assert isinstance(self.model, supported_models)
